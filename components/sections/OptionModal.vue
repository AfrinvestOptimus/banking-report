<template>
  <transition appear name="fade">
    <div
      class="modal fixed bg-[rgba(0,0,0,.7)] text-white top-0 bottom-0 right-0 left-0 flex justify-center items-center px-4 md:px-0"
    >
      <div class="modal-box bg-white py-8 md:py-20 px-6 md:px-10">
        ...
        <h2
          class="uppercase text-primary font-inter text-xl md:text-2xl font-bold mb-6"
        >
  ...
  methods: {
    selectFormat(itemSelected) {
      this.types.forEach((type) => {
        type.selected = type === itemSelected
      })
